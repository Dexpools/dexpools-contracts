...
contract TradeManager is TradeOwner, ReentrancyGuard {
  ...
  function createNewTrade(
    ...
    require(trades[tradeKey].isExist != true, "the post trade is already exist");
  ...
  function fillTrade(
    ...
    require(trades[tradeKey].isExist, "the post trade is not exist");
    require(trades[tradeKey].user0 != _msgSender(), "user0 should be msgSender");
    require(trades[tradeKey].status == Status.Unfilled, "trade status should be unfilled");
    require(_tokenIsAccepted(acceptedToken, trades[tradeKey].acceptedTokens) == true, "the token should be accepted");

    require(token0FillAmount <= trades[tradeKey].token0AmountRemaining, "token0 fill amount should be smaller than remaining amount");
    ...
    if (acceptedToken == address(0)) {
      require(msg.value >= fillAcceptedObj.acceptedTokenAmountSent, "msg value should be greater than accpeted amount");
    } else {
      // check for tax tokens where amount received != transfer amount
    ...
    require(fills[fillKey].isExist != true, "fills should not be exist");
  ...
  function updateAcceptedTokens(
    ...
    require(trades[tradeKey].isExist, "the post trade is not exist");
    require(trades[tradeKey].user0 == _msgSender(), "user0 should be msgSender");
    require(trades[tradeKey].status == Status.Unfilled, "the status should unfilled");
    require(acceptedTokens.length == acceptedAmounts.length, "accepted tokens and amounts length should be match");
    require(_validAcceptedTokens(trades[tradeKey].token0, acceptedTokens) == true, "the accepted tokens should be valid");
    require(_validAcceptedAmounts(acceptedAmounts) == true, "accepted token amounts should be valid");
  ...
  function cancelTrade(
    ...
    string memory tradeKey = _returnTradeKey(tradeId, _msgSender());
    require(trades[tradeKey].isExist, "the post trade is not exist");
    require(trades[tradeKey].status == Status.Unfilled, "the trade status should be unfilled");
    require(trades[tradeKey].user0 == _msgSender(), "user0 should be msgSender");
    ...
    if (trades[tradeKey].token0 == address(0)) {
      ...
      require(IERC20(trades[tradeKey].token0).balanceOf(address(this)) >= trades[tradeKey].token0AmountRemaining, "the token balance should be greater than remaining token0Amount");
